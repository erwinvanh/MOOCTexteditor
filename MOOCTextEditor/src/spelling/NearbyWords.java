...
public class NearbyWords implements SpellingSuggest {
	...
	public void subsitution(String s, List<String> currentList, boolean wordsOnly) {
		...
		for(int index = 0; index < s.length(); index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				//System.out.println("Substitution : " + sb.toString());
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index < s.length(); index++){
			...
			//System.out.println("Deletion : " + sb.toString());
		...
		while (queue.size() > 0){
			...
			for (String neighbour : n){
				if ( ! visited.contains(neighbour)){
					...
					// Quit if threshold is reached
					if ( visited.size() == THRESHOLD){
						System.out.println("Reached treshold");
						return retList;
					}
					...
					if (dict.isWord(neighbour)){
						...
						numSuggested ++ ;
						if ( numSuggested == numSuggestions){
							return retList;
						}
					}
				}
