...
public class WPTree implements WordPath {
	...
	public WPTree () {
		this.root = null;
		// TODO initialize a NearbyWords object --> DONE
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		System.out.println("From " + word1 + "  to " + word2 );
		
		Dictionary d = new DictionaryHashSet();
		DictionaryLoader.loadDictionary(d, "data/dict.txt");
		if (!d.isWord(word2)) {
			System.out.println(word2 +"  is not in dictionary" );
			return null;
		}
		...
		while (queue.size() > 0){
			...
			System.out.println("List for " + curr.getWord() + " is " + neighbors);
			for (String n : neighbors){
				if ( ! visited.contains(n)){
					visited.add(n);
					if (visited.size() == THRESHOLD){
						System.out.println("I give up, to far away");
						return null;					
					}
					...
					if (n.equals(word2)){
						System.out.println("Matched : " + newNode.buildPathToRoot());
