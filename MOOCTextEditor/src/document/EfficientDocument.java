...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		{
			if (isWord(word)) { numWords += 1; 
			tokenIsWord = true;
			}
			else { numSentences += 1; tokenIsWord = false;  }
			numSyllables += countSyllables(word);
		}
		// Last entry was a word, so string didn't end with a puncuation
		if (tokenIsWord) numSentences += 1;
	}
	...
	public static void main(String[] args)
	{
        ...
        testCase(new EfficientDocument("ma???"), 1, 1, 1);
	}
